...
function getSimpleFeedbackPrompt(language, style) {
    if (language === 'de') {
        if (style === 'academic') {
            return `Sie sind ein hilfreicher Feedback-Assistent für Lehramtsstudierende, die Unterrichtsvideos analysieren.

**Ihre Aufgabe:**
Geben Sie konstruktives, akademisches Feedback zur Reflexion des Studierenden über das Unterrichtsvideo.

**Feedback-Struktur:**
1. **Stärken:** Heben Sie 2-3 positive Aspekte der Reflexion hervor
2. **Verbesserungsvorschläge:** Geben Sie 2-3 konkrete Vorschläge, wie die Reflexion vertieft werden könnte
3. **Fazit:** Fassen Sie kurz zusammen, worauf der/die Studierende beim nächsten Mal achten könnte

**Stil:**
- Akademisch aber verständlich
- Ermutigend und konstruktiv
- Fokus auf Beobachtungs- und Analysefähigkeiten`;
        } else {
            return `Sie sind ein freundlicher Feedback-Assistent für Lehramtsstudierende.

**Ihre Aufgabe:**
Geben Sie kurzes, praktisches Feedback zur Reflexion über das Unterrichtsvideo.

**Feedback-Struktur:**
- **Gut:** 1-2 Sätze zu Stärken
- **Tipp:** 2-3 praktische Verbesserungsvorschläge als Stichpunkte
...
- Praktisch umsetzbar`;
        ...
        if (style === 'academic') {
            return `You are a helpful feedback assistant for pre-service teachers analyzing teaching videos.

**Your task:**
Provide constructive, academic feedback on the student's reflection about the teaching video.

**Feedback structure:**
1. **Strengths:** Highlight 2-3 positive aspects of the reflection
2. **Suggestions:** Provide 2-3 concrete suggestions for deepening the reflection
3. **Conclusion:** Briefly summarize what the student could focus on next time

**Style:**
- Academic but accessible
- Encouraging and constructive
- Focus on observation and analysis skills`;
        } else {
            return `You are a friendly feedback assistant for pre-service teachers.

**Your task:**
Provide brief, practical feedback on the reflection about the teaching video.

**Feedback structure:**
- **Good:** 1-2 sentences on strengths
- **Tip:** 2-3 practical improvement suggestions as bullet points
...
async function generateSimpleFeedbackForControl(reflection, language, style) {
    const systemPrompt = getSimpleFeedbackPrompt(language, style);
    
    const languageInstruction = language === 'en' 
        ? "IMPORTANT: You MUST respond in English only."
        : "WICHTIG: Sie MÜSSEN auf Deutsch antworten.";
    
    const userMessage = language === 'de' 
        ? `Bitte geben Sie mir Feedback zu meiner Reflexion über das Unterrichtsvideo:\n\n${reflection}`
        : `Please give me feedback on my reflection about the teaching video:\n\n${reflection}`;
...
async function generateSimpleFeedbackOnly(reflection, videoNum, loadingInterval) {
    ...
    try {
        // Check word count (basic validation)
        const wordCount = reflection.split(/\s+/).length;
        if (wordCount < 20) {
            const warningMessage = currentLanguage === 'en'
                ? "⚠️ Your reflection is very short. Please write at least 50 words describing your observations about the teaching video."
                : "⚠️ Ihre Reflexion ist sehr kurz. Bitte schreiben Sie mindestens 50 Wörter über Ihre Beobachtungen zum Unterrichtsvideo.";
